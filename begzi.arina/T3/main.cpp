...
double area(const Polygon& poly) {
    long long sum = std::accumulate(poly.points.begin(), poly.points.end(), 0LL,
    ...
    while (std::getline(std::cin, command)) {
        ...
        if (main_cmd == "AREA") {
            ...
            if (polygons.empty()) {
                ...
                std::cout << area(*std::max_element(polygons.begin(), polygons.end(),
                    AreaCompare())) << "\n";
            }
            else if (sub == "VERTEXES") {
                std::cout << std::max_element(polygons.begin(), polygons.end(),
                    NumOfVertexesCompare())->points.size() << "\n";
            }
            else {
                std::cout << "<INVALID COMMAND>\n";
            }
            ...
            if (polygons.empty()) {
                ...
                std::cout << area(*std::min_element(polygons.begin(), polygons.end(),
                    AreaCompare())) << "\n";
            }
            else if (sub == "VERTEXES") {
                std::cout << std::min_element(polygons.begin(), polygons.end(),
                    NumOfVertexesCompare())->points.size() << "\n";
            }
            else {
                std::cout << "<INVALID COMMAND>\n";
            ...
            if (!(iss >> target)) {
                ...
                std::cout << std::count(polygons.begin(), polygons.end(), target) << "\n";
            ...
            if (!(iss >> target)) {
                ...
                MaxSeqState res = std::accumulate(polygons.begin(), polygons.end(),
                    MaxSeqState{ 0, 0 },
