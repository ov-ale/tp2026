...
int main() {
    try {
        ...
        shapes.push_back(std::make_unique<IsoscelesTrapezoid>(
            Point(0, 0), 8.0, 4.0, 3.0));
        shapes.push_back(std::make_unique<IsoscelesTrapezoid>(
            Point(2, 1), 6.0, 3.0, 2.0));
        ...
        composite->addShape(std::make_unique<IsoscelesTrapezoid>(
            Point(8, 2), 5.0, 3.0, 2.0));
        ...
        std::cout << "\n--- SCALING BY FACTOR " << std::fixed
                  << std::setprecision(1) << factor << " ---" << std::endl;
